...
#SBATCH -p cclake
#SBATCH --mem=16000
#SBATCH -o logs/merge_vcfs_%j.out
#SBATCH -e logs/merge_vcfs_%j.err
...
# Gather per-chromosome VCFs
VCF_ARGS=""
for CHR in "${CHROMOSOMES[@]}"; do
    VCF="${BASE_DIR}/vcf/${FAMILY}_cohort_chr_${CHR}.vcf.gz"
...
gatk --java-options "-Djava.io.tmpdir=${TMP_DIR} -Xmx14G" MergeVcfs \
    ${VCF_ARGS} \
    -O ${BASE_DIR}/vcf/${FAMILY}_cohort.vcf.gz

[ -f "${BASE_DIR}/vcf/${FAMILY}_cohort.vcf.gz" ] || { echo "ERROR: Merged cohort VCF not created"; exit 1; }
