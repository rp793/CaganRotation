...
# Determine sample indices (assuming order: CHILD, FATHER, MOTHER)
...
[ $? -eq 0 ] && [ -s "$TEMP_VCF" ] || { echo "Error: Compression failed for $TEMP_VCF"; rm -f "$TEMP_VCF.uncompressed"; exit 1; }
...
[ $? -eq 0 ] || { echo "Error: Failed to index $TEMP_VCF"; rm -f "$TEMP_VCF.uncompressed" "$TEMP_VCF"; exit 1; }
