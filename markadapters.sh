...
[ -f "${BASE_DIR}/rg/${SAMPLE}.unaligned.bam" ] || { echo "ERROR: Input BAM not found: ${BASE_DIR}/rg/${SAMPLE}.unaligned.bam"; exit 1; }
...
gatk --java-options "-Djava.io.tmpdir=${TMP_DIR} -Xmx4G" MarkIlluminaAdapters \
    --INPUT ${BASE_DIR}/rg/${SAMPLE}.unaligned.bam \
    --OUTPUT ${BASE_DIR}/rg/${SAMPLE}.adaptMarked.bam \
    --METRICS ${BASE_DIR}/metrics/${SAMPLE}.adaptMarked.metrics.txt

[ -f "${BASE_DIR}/rg/${SAMPLE}.adaptMarked.bam" ] || { echo "ERROR: Output BAM not created: ${BASE_DIR}/rg/${SAMPLE}.adaptMarked.bam"; exit 1; }
