...
BQSR="/home/rp793/rds/hpc-work/genomes/dog/Genomes/3.1/variants/canfam3renamed.vcf.gz"
...
[ -f "$BQSR" ] || { echo "ERROR: Known sites VCF not found: $BQSR"; exit 1; }
...
gatk --java-options "-Djava.io.tmpdir=${TMP_DIR} -Xmx8G" BaseRecalibrator \
    -R ${REFERENCE} \
    -I ${BASE_DIR}/${SAMPLE}.sorted.bam \
    --use-original-qualities \
    -O ${BASE_DIR}/${SAMPLE}.bsqr.txt \
    --known-sites ${BQSR}

[ -f "${BASE_DIR}/${SAMPLE}.bsqr.txt" ] || { echo "ERROR: Output recalibration table not created: ${BASE_DIR}/${SAMPLE}.bsqr.txt"; exit 1; }
